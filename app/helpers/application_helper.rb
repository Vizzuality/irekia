module ApplicationHelper
  ...
  def message_for_facebook(content)
    if content and content.text
      facebook_message_link = polymorphic_url(content.parent || content).gsub(/datalogger=true/, '')
      content.text.truncate(139 - facebook_message_link.length) + ' ' + facebook_message_link
    end
  end
  ...
  def translate_field(model, field)
    ...
    return alternate_locale_value if alternate_locale_value.present?
    return no_translated_value    if no_translated_value.present?
    ''
  rescue
    ''
  end
